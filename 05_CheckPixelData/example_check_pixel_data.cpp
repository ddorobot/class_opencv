...
int main(void)
{
	...
	while(1)
	{
		if( !read_image.empty() )
		{
			...
			{
				unsigned char blue = (unsigned char)read_image.data[((g_mouse_y*read_image.cols) + g_mouse_x)*3+0] ;
				unsigned char green = (unsigned char)read_image.data[((g_mouse_y*read_image.cols) + g_mouse_x)*3+1] ;
				unsigned char red = (unsigned char)read_image.data[((g_mouse_y*read_image.cols) + g_mouse_x)*3+2] ;
